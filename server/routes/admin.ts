    ...
    const analytics = await withDB(
      async () => {
        ...
        const [
          ...
          Booking.aggregate([
            { $match: { paymentStatus: "completed", createdAt: { $gte: previousStartDate, $lt: startDate } } },
            { $group: { _id: null, total: { $sum: "$totalAmount" } } }
          ]).then(result => result[0]?.total || 0),
          ...
          Booking.countDocuments({ status: "confirmed", createdAt: { $gte: startDate } }),
          Booking.countDocuments({ status: "pending", createdAt: { $gte: startDate } }),
          Booking.countDocuments({ status: "cancelled", createdAt: { $gte: startDate } }),

          // User stats
          User.countDocuments({ createdAt: { $gte: startDate } }),
          User.countDocuments({ isActive: true, lastActivity: { $gte: startDate } }),
          User.countDocuments({ createdAt: { $gte: startDate } }),
        ]);
        ...
        return {
          revenue: {
            total: totalRevenue,
            monthly: monthlyRevenue.reduce((sum, month) => sum + month.revenue, 0),

...
router.post('/initiate', async (req, res) => {
  try {
    const { amount, productName, transactionId, method, customerInfo } = req.body;

    if (!amount || !productName || !transactionId || !method) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing required fields: amount, productName, transactionId, method' 
      });
    }

    console.log(`Initiating ${method} payment:`, { amount, productName, transactionId });
...
router.post('/verify/khalti', async (req, res) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Khalti verification failed: ${JSON.stringify(errorData)}`);
    }

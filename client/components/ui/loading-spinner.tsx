...
export function LoadingSpinner({ 
  ...
  const getSpinnerComponent = () => {
    switch (variant) {
      ...
      case "bounce":
        return (
          <div className="flex space-x-1">
            <div className={cn("bg-kanxa-blue rounded-full animate-bounce", 
              size === "sm" ? "w-2 h-2" : 
              size === "md" ? "w-3 h-3" : 
              size === "lg" ? "w-4 h-4" : "w-6 h-6"
            )} style={{ animationDelay: "0ms" }} />
            <div className={cn("bg-kanxa-orange rounded-full animate-bounce", 
              size === "sm" ? "w-2 h-2" : 
              size === "md" ? "w-3 h-3" : 
              size === "lg" ? "w-4 h-4" : "w-6 h-6"
            )} style={{ animationDelay: "150ms" }} />
            <div className={cn("bg-kanxa-green rounded-full animate-bounce", 
              size === "sm" ? "w-2 h-2" : 
              size === "md" ? "w-3 h-3" : 
              size === "lg" ? "w-4 h-4" : "w-6 h-6"
      ...
      case "kanxa":
        return (
          <div className={cn("relative", sizeClasses[size])}>
            ...
            <div className="absolute inset-2 border-2 border-transparent border-t-kanxa-orange rounded-full animate-spin" style={{ animationDirection: "reverse" }} />
          </div>
        );
        ...
        return (
          <div className="flex items-center space-x-2">
            <Icon className={cn(
              "animate-spin",
              sizeClasses[size],
              theme === "transportation" ? "text-kanxa-blue" :
              theme === "construction" ? "text-kanxa-orange" :
              theme === "garage" ? "text-kanxa-green" :
              "text-kanxa-blue"
            )} />
            {theme && (
              <div className={cn("flex space-x-1", 
                ...
                <div className={cn("rounded-full animate-pulse bg-current opacity-60",
                  size === "sm" ? "w-1 h-1" :
                  size === "md" ? "w-1.5 h-1.5" :
      ...
      default:
        return (
          <Loader2 className={cn("animate-spin text-kanxa-blue", sizeClasses[size])} />
        );
    }
  };

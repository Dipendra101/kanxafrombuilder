...
// import {
//   User, Mail, Phone, MapPin, Calendar, Edit, Save, Bell, Shield, CreditCard, Settings, LogOut, Camera, Upload, X, CheckCircle, KeyRound, Loader2
// } from "lucide-react";
...
//   // Fixed password form hook with proper typing
//   const {
...
//   // Fixed password change handler with proper typing
...
//   const onEmailVerificationSubmit = (data: EmailVerificationFormData) => {
//     // Update the email in profile first if changed, then send verification
...
//   const handleSendVerification = () => sendVerificationMutation.mutate();
...
//             <img 
//               src={profileData?.avatarUrl || `https://ui-avatars.com/api/?name=${profileData?.name}&background=random`} 
//               alt="Profile" 
//               className="w-24 h-24 object-cover rounded-full mx-auto mb-4 border-2 border-white/30"
//             />
...
//                                 <Input id="company" {...register("company")} disabled={!isEditing} />
//                               </div>
//                           </div>
//                           <div>
...
//                             <Textarea id="bio" {...register("bio")} disabled={!isEditing} rows={3} className="resize-none" />
//                           </div>
//                       </CardContent>
//                     </Card>
//                 </form>
...
//                                     <CreditCard className="h-5 w-5 text-kanxa-orange" />
//                                   }
//                                 </div>
//                                 <div>
...
//                           <div className="text-3xl font-bold text-kanxa-blue mb-2">{loyaltyData.totalBookings}</div>
//                           <div className="text-sm text-gray-600">Total Bookings</div>
//                         </CardContent>
//                       </Card>
//                       <Card className="text-center">
//                         <CardContent className="p-6">
//                           <div className="text-3xl font-bold text-kanxa-orange mb-2">{loyaltyData.totalOrders}</div>
//                           <div className="text-sm text-gray-600">Total Orders</div>
//                         </CardContent>
//                       </Card>
//                       <Card className="text-center">
//                         <CardContent className="p-6">
//                           <div className="text-3xl font-bold text-kanxa-green mb-2">NPR {loyaltyData.totalSpent.toLocaleString()}</div>
//                           <div className="text-sm text-gray-600">Total Spent</div>
//                         </CardContent>
//                       </Card>
//                       <Card className="text-center">
//                         <CardContent className="p-6">
//                           <div className="text-3xl font-bold text-kanxa-navy mb-2">{loyaltyData.loyaltyPoints}</div>
...
//                           Membership Status 
//                           <Badge className="bg-yellow-100 text-yellow-800">{loyaltyData.membershipLevel}</Badge>
//                         </CardTitle>
//                       </CardHeader>
//                       <CardContent>
//                         <p className="text-sm text-gray-600">You are making great progress towards the next level!</p>
//                       </CardContent>
//                   </Card>
//               </TabsContent>
...
//                           </div>
//                         ) : (
//                           <form onSubmit={handleEmailSubmit(onEmailVerificationSubmit)} className="space-y-4 mb-6">
//                             <div>
...
//                               <p className="text-sm text-gray-600">Get notified about special deals</p>
//                             </div>
//                             <Switch 
...
//                               <p className="text-sm text-gray-600">Monthly newsletter with updates</p>
//                             </div>
//                             <Switch 
...
export default function Profile() {
  ...
  // Fetch other data with initial defaults
  const { data: activityData = [], isLoading: isActivityLoading } = useQuery({
    queryKey: ['activity'],
    queryFn: async () => (await userAPI.getActivity()).activity,
    initialData: [],
  ...
  const { data: loyaltyData = { totalBookings: 0, totalOrders: 0, totalSpent: 0, loyaltyPoints: 0, membershipLevel: "Bronze" }, isLoading: isLoyaltyLoading } = useQuery({
    queryKey: ['loyalty'],
    queryFn: async () => (await userAPI.getLoyalty()).loyalty,
    initialData: { totalBookings: 0, totalOrders: 0, totalSpent: 0, loyaltyPoints: 0, membershipLevel: "Bronze" },
  });

  const { data: notifications = { bookingUpdates: true, promotions: false, serviceReminders: true, newsletter: true }, isLoading: isNotificationsLoading } = useQuery({
    queryKey: ['notifications'],
    queryFn: async () => (await userAPI.getNotifications()).notifications,
    initialData: { bookingUpdates: true, promotions: false, serviceReminders: true, newsletter: true },
  });
  ...
  // Form setup for profile info with react-hook-form & zod
  const {
    register,
    handleSubmit,
    reset,
  ...
  // Reset form whenever profileData changes (like after mutation)
  useEffect(() => {
    reset({
      name: profileData?.name || '',
      email: profileData?.email || '',
  ...
  // Update email if changed then send verification email
  const onEmailVerificationSubmit = (data: EmailVerificationFormData) => {
    if (data.email !== profileData?.email) {
      updateProfileMutation.mutate(
    ...
    return (
      <Layout>
        <div className="container py-12">
          <Card>
            <CardContent className="h-96 w-full animate-pulse bg-gray-200 rounded" />
          </Card>
        </div>
      </Layout>
    );
                ...
                <CardContent className="space-y-4">
                  {activityData.map((activity: any) => (
                    <div key={activity.id} className="flex items-center justify-between p-3 border rounded-lg">
                      ...
                      <Badge
                        className={`${
                          activity.status === "completed" || activity.status === "delivered" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
                        }`}
                      >
                        {activity.status}
                      </Badge>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-medium">Service Reminders</p>
                      <p className="text-sm text-gray-600">Receive reminders for your service appointments</p>
                    </div>
                    <Switch
                      checked={notifications.serviceReminders}
                      onCheckedChange={(checked) => handleNotificationChange("serviceReminders", checked)}
                  ...
                  <form onSubmit={handlePasswordSubmit(onPasswordChange)} className="space-y-4">
                    ...
                    <Button type="submit" disabled={passwordMutation.isLoading}>
                      {passwordMutation.isLoading ? (
                        <>
                          <Loader2 className="inline animate-spin mr-2 h-4 w-4" /> Changing...
                        </>
                      ) : (
                        "Change Password"
                      )}
              ...
              <Card>
                ...
                <CardContent>
                  <form onSubmit={handleEmailSubmit(onEmailVerificationSubmit)} className="space-y-4">
                    <div>
                      <Label htmlFor="emailVerificationEmail">Email</Label>
                      <Input id="emailVerificationEmail" {...registerEmail("email")} />
                      {emailErrors.email && <p className="text-red-500 text-sm mt-1">{emailErrors.email.message}</p>}
                    </div>
                    <Button type="submit" disabled={sendVerificationMutation.isLoading}>
                      {sendVerificationMutation.isLoading ? (
                        <>
                          <Loader2 className="inline animate-spin mr-2 h-4 w-4" /> Sending Code...
                        </>
                      ) : (
                        "Send Verification Code"
                      )}
                  ...
                  <div className="mt-4 flex gap-2 items-center">
                    ...
                    <Button
                      onClick={handleVerifyCode}
                      disabled={verifyCodeMutation.isLoading || verificationCode.length !== 6}
                    >
                      {verifyCodeMutation.isLoading ? (
                        <>
                          <Loader2 className="inline animate-spin mr-2 h-4 w-4" /> Verifying...
                        </>
                      ) : (
                        "Verify Code"
                      )}
